...
## 🎨 Design system (redesign M22 · 2026-07-22 — reuse ก่อนเขียนใหม่ ให้ต่อยอดง่าย)
- ธีม: **light-only** (dark พักไว้ — เว็บโลโก้แบรนด์เยอะไม่เหมาะมืด) · ฟอนต์ **Kanit** (`--font-kanit`) · ตัวเลขใช้คลาส **`.tnum`** (Geist Mono, tabular) เสมอกับ ราคา/ปี/จำนวน
- **สี = token ใน `globals.css`** เท่านั้น (อย่า hardcode hex): `accent`(Royal) · `success/warning/danger` · `pt-diesel/petrol/hybrid/ev` (สื่อ "หมวดขุมพลัง" ไม่ใช่คะแนนรถ — ต้องมีข้อความคู่เสมอ, a11y)
- **UI primitives อยู่ `src/components/badges.tsx`** (แหล่งเดียว — อย่าสร้างซ้ำในหน้า): `Badge`ไม่มี · `ConfidenceBadge` · `LifecycleBadge` · `LifecycleDot` · `PowertrainDots`/`ptDotClass` · `Chip` · `FreshnessTag` · `DataStatusValue` · `EvidenceLink`
- ตารางรุ่นรถทุกหน้า = `car-database-explorer.tsx` (home + brand hub) · หน้ารุ่น = `cars/[slug]` · แหล่งอ้างอิง = `sources-section.tsx`
- เพิ่มหน้า/ฟีเจอร์ใหม่ → import primitives + token ข้างบน · ห้ามเขียน badge/chip/สี ใหม่รายหน้า (กัน design drift)
