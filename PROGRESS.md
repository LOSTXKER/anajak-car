...
อัปเดตล่าสุด: 2026-08-30 (**M31b หน้าแรกฟีลเว็บเช็คราคาหุ้น (ไม่มีรูปรถ + กราฟ + โลโก้แบรนด์ใน hero) — รอเบสเคาะ · branch `proto/redesign-2026`**)

## รอบล่าสุด (M31b — หน้าแรกฟีลเว็บเช็คราคาหุ้น · 2026-08-30 ดึก)
...
## รอบก่อนหน้า (M31 — หน้าแรกพรีเมียม · 2026-08-30 ดึก)
M30 ก็ยังไม่ผ่าน ("แบบเก่ายังดีกว่า") → **หยุดเดา แล้วถามตรงๆ 2 ข้อ** ได้คำตอบที่เปลี่ยนทุกอย่าง:
- ของเดิมที่ของใหม่ทำหาย = **พาดหัวใหญ่ + ตัวหนังสือพิมพ์วน (typewriter)** และ **ช่องค้นหาใหญ่กลางหน้า** (ผมตัดทิ้งทุกรอบ)
- ความรู้สึกที่เบสอยากได้ = **พรีเมียม ดูแพง เหมือนเว็บรถหรู** (Porsche/Lexus/Tesla) — ไม่ใช่แดชบอร์ดข้อมูลแน่น · ref dotabuff/arena ที่เคยส่งมา = ชอบ "ความมีข้อมูล" ไม่ใช่หน้าตา
- **`/proto/home-2026/lux`** = หน้าแรกใหม่: เก็บ `HeroHeadline` (typewriter ตัวจริงของเว็บ) + ช่องค้นหาใหญ่ · **ภาพรถเต็มเวทีเป็นพระเอก** + เงาใต้ท้องรถ · แถบแบรนด์โลโก้ใหญ่ช่องไฟกว้าง (ไม่ใช่การ์ด) · รุ่นเด่นภาพใหญ่สลับซ้าย-ขวาแบบเว็บผู้ผลิต · ตารางทั้งฐานเส้นบางเนี้ยบ · ท้ายหน้าบอกชนิดราคา+สิ่งที่ยังไม่มี
- หน้าเทียบ `/proto/home-2026` มี 3 ปุ่ม: **พรีเมียม (ใหม่)** / หน้าแรกวันนี้ / แบบข้อมูลจัด (M30)
- verify: lint + tsc + build ผ่าน · detector สกิล 0 finding · วัดจริง: ไม่มี overflow · hero 865px · แบรนด์ 3 · รุ่นเด่น 3 · ตาราง 12 แถว · แก้ error `onSubmit` ใน server component · แก้คำบรรยายภาพทับตัวรถบนมือถือ
- ⚠️ ข้อจำกัดที่ยังอยู่: รูปรถมีเฉพาะ Toyota 5 รุ่น (Tesla/Benz ยังไม่มีสิทธิ์ใช้ภาพ) — ลุคพรีเมียมพึ่งภาพมาก ถ้าเบสเอาทางนี้ ต้องเคลียร์เรื่องสิทธิ์ภาพให้ครบก่อนขึ้นจริง

## รอบล่าสุด (M30 — หน้าแรกอย่างเดียว ออกแบบจากความจริงของผลิตภัณฑ์ · 2026-08-30 ค่ำ)
เบสตี M29 ว่า **"ดู AI slop"** แล้วถามกลับว่า *"คุณรู้มั้ยว่าเว็บเราเกี่ยวกับอะไร"* → จุดที่ผิดคือ 3 รอบแรกไปหา "โลกภาพ" มาสวมทับข้อมูลชุดเดิม (แดชบอร์ด/แค็ตตาล็อก/คู่มือ/แผนที่ BTS) แทนที่จะเริ่มจากความจริงของ CARMETA · เบสยืนยันความเข้าใจ (แยกรถถึงรุ่นย่อยจริง · ทุกตัวเลขมีที่มา · ประวัติไม่เขียนทับ · ไม่มีข้อมูลก็บอกว่าไม่มี) และสั่ง **"ทำมาแค่หน้าแรกก่อน"**
- **`/proto/home-2026`** = หน้าเทียบ (หน้าแรกใหม่ ↔ หน้าแรกวันนี้) · **`/proto/home-2026/view`** = หน้าแรกจริง
- แนวคิด: **ความลึกกลายเป็นภาพ** — ทุกรุ่นแสดง**จุดครบทุกรุ่นย่อย (64 จุด)** บน**แกนราคาเส้นเดียวทั้งหน้า** (สเกลเท่าตัว 5 แสน→8 ล้าน เพราะราคาห่างกัน 13 เท่า · เขียนกำกับไว้ท้ายหน้า) → เห็นทันทีว่า Hilux Travo 18 เกรดกระจายเกือบเท่าตัว ส่วน bZ4X 2 เกรดชิดกัน · แบรนด์เป็นหัวข้อของกลุ่ม (ทางเข้าหลักตามที่เบสสั่ง) · วันที่ตรวจ+ที่มาติดทุกแถว · ท้ายหน้าบอกตรงๆ ว่าอะไรยังไม่มี
- ตัดของที่ทำให้ดูเป็นงาน AI ออกหมดตามเช็กลิสต์ craft: ไม่มีการ์ดกริดขนาดเท่ากัน · ไม่มีแถบสถิติ 4 ช่อง · ไม่มี gradient · ไม่มี eyebrow เหนือหัวข้อ · ไอคอนเป็น SVG วาดเอง ไม่ใช่ emoji · ธีม selection/focus-visible เอง
- ข้อมูลใหม่ใน fixture: `variantPrices` = ราคารุ่นย่อยครบ 64 รายการ (ดึงจาก seed จริง + phase7 Tesla/Benz)
- ใช้สกิล `impeccable` เต็มกระบวน (context → new-work → concept-seed → decision page → craft-floor) · เบส re-roll หนึ่งครั้งแล้วเปลี่ยนทิศเป็น "ออกแบบจากข้อมูล" แทนการเลือกโลกภาพ
- verify: lint + tsc + build ผ่าน · detector ของสกิล = 0 finding · วัดจริงในเบราว์เซอร์: ไม่มี overflow แนวนอน · 12 แถว/64 จุดครบ · แก้ contrast ข้อความรอง 3.8:1 → 4.6:1 · แก้หัวเว็บล้นบนมือถือ · แก้ tick แกนล้นขอบ · แก้ราคาช่วงซ้ำของรุ่นเกรดเดียว (EQS)

## รอบล่าสุด (M29 — brand-first + ลุคสว่าง 2 แบบ · 2026-08-30 เย็น)
เบสดู M28 แล้วให้ฟีดแบ็กสองข้อที่เปลี่ยนโจทย์: (1) **"คนชอบเลือกหมวดหมู่มากกว่าค้นหา"** (2) **"แบบปัจจุบันยังดีกว่า"** → ถามเจาะได้ว่า หมวดหลัก = **แบรนด์** · หน้าตา = **ยังอยากฉีก แต่พื้นสว่างเท่านั้น** · ระหว่างทางเบสส่ง ref เพิ่ม: **artificialanalysis.ai / arena.ai** (dashboard เทียบข้อมูลพื้นสว่าง)
- **`/proto/brand-first`** = หน้าเทียบรอบ 3 · ทุกทาง **เปิดหน้าแรกด้วยการ์ดแบรนด์** · เพิ่ม **หน้าแบรนด์** เป็นหน้าจอใหม่ (flow: แบรนด์ → รุ่น → รุ่นย่อย) · 4 หน้าจอ × คอม/มือถือ × สวิตช์ข้อมูลไม่ครบ
- **G · ANALYTICS** (ขาว · Royal #2f56c9 + ฟ้า #0ac3fb): ตาม ref ที่เบสส่ง — การ์ดแบรนด์ + **กราฟแท่งราคาเริ่มต้นทุกรุ่นบนแกนร่วม** + **ตารางจัดอันดับกดเรียงได้จริง** (client) · หน้ารุ่นมี **แผนภาพจุด "จ่ายเพิ่มแล้วได้อะไร"** (แกนนอน=ราคา แกนตั้ง=จำนวน ADAS ที่ยืนยัน — เพราะไลน์นี้กำลัง 204 PS เท่ากันทุกคัน) + บันไดราคา
- **H · BLUEPRINT** (กระดาษฟ้า #eef3fc · หมึก #1345c4): ต่อยอดจาก M28 + หน้าแบรนด์เต็ม + หน้าแรกเป็นการ์ดแบรนด์
- ทิศ "ปัจจุบัน" มีหน้าแบรนด์ด้วย (`current-brand.tsx` เลียนหน้าจริง ใช้ StatBar/SectionHeader/BrandMark ตัวจริง)
- ข้อมูลแบรนด์จริงเพิ่มใน fixture: Toyota (TMT · ตั้งแต่ 1962 · ดีลเลอร์ 152 ราย/โชว์รูม 447) · Tesla/Benz ไม่มีข้อมูลผู้จำหน่าย/ปี → แสดง "ไม่มีข้อมูล" (สถานะขอบของหน้าแบรนด์ = Tesla)
- verify: lint + tsc + build ผ่าน · เปิดจริงทุกลุค × 4 หน้าจอ × คอม/มือถือ · แก้ระหว่างทาง: หัวข้อกราฟให้ตรงกับแกนจริง · โลโก้ wordmark ล้นกล่อง · ป้ายแกนกราฟทับกันบนมือถือ

## รอบล่าสุด (M28 — หน้าลองเลือกลุค · 2026-08-30 บ่าย)
เบสดู M27 แล้วบอก **"ยังไม่โดน"** → เจาะได้ว่า M27 เทียบแต่ *ลำดับการเดิน* แต่ทุกทางใช้หน้าตาชุดเดียวกัน · เบสสั่งเพิ่ม: กลุ่มเป้าหมาย = คนศึกษาข้อมูลรถเพื่อตัดสินใจซื้อ · อยากได้ฟีล dotabuff/op.gg/prydwen · **สร้างสรรค์ ฉีกจากเดิม ไม่ยึดของเดิม** · สีน้ำเงินยังเป็น CI (โทนใหม่ได้)
- **`/proto/look-2026`** = หน้าเทียบ "หน้าตา" 3 ลุคใหม่ + ปัจจุบันเป็นตัวตั้ง · ครบ 3 หน้าจอ × คอม/มือถือ × สวิตช์ข้อมูลไม่ครบ · ทุกลุคใช้ข้อมูลจริงชุดเดียวกัน (`_kit/data.ts`) ต่างกันที่หน้าตาล้วนๆ
- **D · META** (มืดกรมท่า #070b16 · accent #4d8bff/#29d3ee): ฟีล dotabuff — ตารางแน่น ทุกแถวมี**แท่งช่วงราคาบนสเกลร่วมทั้งฐาน** · หน้ารุ่นมี**บันไดราคา 18 รุ่นย่อยเป็นแท่ง** เห็นช่องว่างราคาในภาพเดียว
- **E · PIT** (กรมท่าอมม่วง #0d1030 · accent #6d8bff/#46e0d0): ฟีล prydwen — รถเป็น**การ์ดจำหน้าได้** (ภาพเด่น แถบสีตามขุมพลัง) + สารบัญเกาะซ้ายทุกหน้า
- **F · BLUEPRINT** (กระดาษฟ้า #eef3fc เส้นตาราง 28px · หมึก #1345c4): **แบบแปลนวิศวกรรม** — เลขข้อกำกับหัวข้อ ตารางจุดไข่ปลา กรอบมุมตัด **ช่องรับรองข้อมูล**ปิดท้ายทุกหน้า (หน้าตาพูดแทนจุดยืน "ตรวจสอบได้")
- โค้ด: `src/app/proto/look-2026/` (looks.css กำหนดตัวแปร `--lk-*` ต่อลุค · `_looks/{meta,pit,blueprint,kit}.tsx`) — **ไม่แตะ globals.css หรือโค้ดของจริง**
- ค้างถ้าเลือกทางมืด: โลโก้แบรนด์สีเข้ม (Ford/Mazda/Isuzu) จมพื้น ต้องมีชิปพื้นอ่อนรอง (ยังไม่ทำในหน้าลอง)
- verify: lint + tsc + build ผ่าน · เปิดจริงครบ 3 ลุค × 3 หน้าจอ × คอม/มือถือ + สถานะข้อมูลไม่ครบ
- **Supabase resume แล้ว** (เบสทำเอง) — เว็บจริงกลับมาเปิดได้ปกติ (`/` และ `/cars/*` ตอบ 200)

## รอบล่าสุด (M27 — หน้าลอง 3 ทิศ + ของเดิมเป็นตัวตั้ง · 2026-08-30)
เบสสั่ง "รื้อ UXUI ใหม่ ใช้งานจริง สวย ง่าย ว้าว SEO ดี · flow ใหม่ก็ได้ · ทำ proto ให้ดู" → ทำหน้าลองตามสกิล /proto (ไม่แตะโค้ดของจริงเลย — `git status` ยืนยันแตะแค่ `src/app/proto/`)
- **`/proto/redesign-2026`** = หน้าเทียบ: เลือกทิศ × เลือกหน้าจอ (หน้าแรก/หน้ารุ่น/หน้ารุ่นย่อย) × สวิตช์ "ข้อมูลไม่ครบ" · คอม (ย่อจาก 1280) กับมือถือ 390 อยู่ในสายตาเดียวกัน · ลิงก์พกตัวเลือกได้ (`?v=&s=&thin=`) · ในกรอบเดินต่อได้จริงทุกหน้า
- **4 ทิศ** (ต่างที่ "ลำดับการเดินของคนใช้" ไม่ใช่สี): `current` ของวันนี้ · `filter` A ตารางคือหน้าแรก (กรองได้จริง + ทุกชุดตัวกรอง = หน้า SEO) · `answer` B ถามแล้วตอบ (ค้นสดขณะพิมพ์ลึกถึงรุ่นย่อย + กล่องคำตอบคำนวณสดจากข้อมูล) · `guide` C คู่มือรถ (สารบัญซ้ายทุกหน้า + ในหน้านี้ขวา — ต่อยอดภาษา M25g)
- ข้อมูลในหน้าลอง = **แช่แข็งจากของจริง** (`src/app/proto/_kit/data.ts`): 12 รุ่น/64 รุ่นย่อย ราคาจริงทั้งหมด + Hilux Travo 18 SKU + ADAS จริง — ไม่ยิง DB (เปิดได้แม้ฐานข้อมูลล่ม)
- **เจอระหว่างทำ 2 เรื่อง**: (1) **ฐานข้อมูล Supabase ของ CARMETA หายไป** — `ENOTFOUND tenant postgres.ijqescdqilxsgseoxwrn`, ไม่อยู่ในลิสต์โปรเจกต์ Supabase ของเบสแล้ว → เว็บจริงเปิดไม่ขึ้นทุกหน้า (2) บั๊กของจริง: หัวหน้ารุ่นบนมือถือ ราคาใน StatBar ถูกตัดท้าย (฿767,00) — จดไว้ในหน้าลอง ยังไม่แก้ของจริง
- verify: lint + tsc + build ผ่าน · เปิดจริงครบ 4 ทิศ × 3 หน้าจอ × คอม/มือถือ + สถานะ "ข้อมูลไม่ครบ" · noindex ติดทุกหน้าใต้ /proto
...
## ▶ NEXT (ทำต่อทันที)
0. **รอเบสเคาะหน้าแรกใหม่ `/proto/home-2026`** → เคาะแล้วค่อยทำหน้าแบรนด์/รุ่น/รุ่นย่อยด้วยภาษาเดียวกัน
