...
- **sidebar ประจำแบรนด์ = เมนูเนื้อหาแบรนด์ "1 หน้า 1 เรื่อง"** (`brand-sidebar.tsx` · แบบหมวดใน prydwen · **ไม่ใช่ลิสต์ชื่อรุ่น**): **ภาพรวมและรุ่นรถ · ไทม์ไลน์และประวัติ · แหล่งอ้างอิง** (มีไอคอน · แต่ละอันเป็นหน้าแยกจริง) · โชว์เฉพาะโซนแบรนด์/รุ่น · โซนไม่ผูกแบรนด์ (`/tierlist`,`/brands` index) = ไม่มี sidebar
- **หน้าแบรนด์แตกเป็น 3 หน้า**: `/brands/[slug]` **ภาพรวมและรุ่นรถ** (ตัวตน+KPI+about+**ตาราง CarDatabaseExplorer เฉพาะแบรนด์นี้** — getBrandDetail.rows brand-scoped · filter URL อยู่หน้านี้) · `/timeline` (getBrandTimeline · Toyota 7 เหตุการณ์) · `/sources` · (เบสสั่งย้ายตารางจาก /models มาภาพรวม → ลบ /models แล้ว)
